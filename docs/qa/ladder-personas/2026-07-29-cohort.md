...
The authoritative journeys run locally at `http://127.0.0.1:3000/ladder` using the deterministic no-key path. The live navigator gate and external AI providers are prohibited for this QA run. Only after those local journeys complete, repeat F02 and L03 as production smoke cases at `https://patient-centered.vercel.app/ladder`; production behavior does not replace or redefine the local result.
...
- Resource oracle: verified `lklp_transportation_region_13` serves Breathitt and must appear as the county-serving transportation route before, or be clearly distinguished from, applicable statewide fallbacks; `ocshcn`, `kentucky_211`, and `kynect_resources` remain reachable as applicable fallbacks.
...
| F04 | 9 | Elementary | McCracken | Small city | English UI; code-switched input | Code-switched narrative | Trade certificate | Description-first with supported profile inference | School and IEP | Statewide school-rights resources | Plan IDEA school-discipline step | First-session | No |
...
| L01 | 5 | Preschool | Fayette | Urban | Spanish | Spanish narrative plus monthly note | Associate degree | Profile-first | School/IEP; parent support | Statewide school resources with Spanish source notice | Persist return check-in, journal, and packet | Longitudinal | No |
| L02 | 8 | Elementary | Breathitt | Rural | English | Concise access narrative | High school; intermittent connectivity | Profile-first | Therapies; transportation | Verified LKLP Region 13 county-serving route before or clearly distinguished from applicable statewide fallbacks | Plan, try, then enroll Kentucky 211 | Longitudinal | No |
| L03 | 10 | Elementary | Rowan | Rural/small town | English | Concise evaluation narrative | College educated | Profile-first | School/IEP; respite only after explicit answer | Statewide evaluation/support resources | Book, barrier, earlier slot, miss, rebook, reload | Longitudinal | Yes |
