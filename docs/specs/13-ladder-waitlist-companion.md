...
**Post-implementation review, 2026-07-25/26.** A six-lens adversarial review (spec fidelity, safety, correctness, determinism, i18n/a11y, cross-cutting invariants) raised 33 findings; each was independently re-verified by two skeptics prompted to refute it, and 22 survived. All 22 were fixed (`93a76bf`→`f84373e`), plus a plural bug found in live testing. Final state: `npm run check` green (**2053 unit tests**), `crisis:gate` PASS, e2e 27 passed / 1 skipped, live-verified in en and es. The three defects worth recording, because each violated something this spec explicitly promised:

1. **The regression watch fired on gains (critical).** The `used to … now` / `antes … ya no` branches had an unconstrained verb slot and accepted a *positive* continuation, and the verb lists carried `us` (matching the pronoun), `use`/`usar`, and `hace`. So "he used to hate the bath but now he loves it" and "ya no usa pañales" raised a skill-loss flag, told the family to phone the clinic, and wrote a permanent "Possible loss of skills" line into the clinician-facing packet — flags have no removal path. Root cause was **this plan's own trap list**, which covered `lost`/`stopped`/`no longer` but never the `used to … now` branch, so the build-breaking corpus was green over the hole. Every branch is now anchored to a named skill verb *in the form that branch takes*, plus a required loss direction; eight gain-traps now guard it. Precision beats recall here by design — the probe backstops what the lexicon misses.
2. **The pulse was unreachable on the path that matters (high).** Answering the skill-loss probe "Yes" unmounted the check-in, so FR-9's one-question experience metric — the whole reason UKHCI's Executive Value Analysis is unblocked — was never asked of exactly the families reporting regression. The sequence position now lives in `FamilyExperience`, so acknowledging the clinic-now card resumes at the pulse instead of restarting (and cannot raise a duplicate flag).
3. **The packet quoted our own questions as the family's words (high).** Needs-screen facts carry the app's question text in `sourceSnippet`, so the packet printed screening questions in quotation marks as caregiver testimony — including ones answered "no". The rule "anything quoted in the packet is a verbatim caregiver sentence" is now enforced and tested.

Also fixed: duplicate facts from orientation follow-up rounds, orientation rounds miscounted as journal notes, regression flags re-firing across rounds, a screen resubmit silently restoring facts the caregiver had excluded, rungs pointing at sections not on the page, legacy `alreadyEnrolled` families still being nagged by the First Steps countdown, an accepted earlier-visit leaving two live bookings, check-in focus/live-region gaps, and no plural handling anywhere ("1 notes"). 11 further findings were rejected under verification as false alarms.
