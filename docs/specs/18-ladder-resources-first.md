...
**Status:** Spec authored 2026-08-04 from direct user feedback on the live flow ("far too much visual clutter… the first thing that should come out is some possible resources… clarification questions would come next"). Extends specs 09 (Family Navigator), 11 (rank-and-justify), 13 (waitlist companion), 15 (structured routing). Ground truth verified against `master` at `2399d2d`. **Implemented on `master` 2026-08-04** — see Implementation Notes below for the eight places the build corrected this spec.

## Implementation Notes (2026-08-04) — where the build corrected the spec

A six-way parallel code survey run before the first edit found several load-bearing errors in this document. The build follows the corrected reading; this section is the record.

1. **The strip must render before a profile exists.** F1 said the strip appears once "a profile and active domains exist", but today's review turn renders on `reviewFacts.length > 0 || reviewDetails` — i.e. *during* the basics turns. Gating the strip on a profile would have made the relocated fact cards, quotes, and confirm buttons unreachable on exactly the path where basics are still being asked, silently breaking the removed-vs-relocated promise. **The strip inherits the review turn's condition verbatim**, and the sentence degrades to whatever is known (omit-unknown).
2. **The "sounds like" sentence cannot be one template.** `tFamily` renders an unmatched `{token}` literally (`family-strings.ts`, and `strings.test.ts` asserts it), so a single `heardStrip` string with optional vars would print `{county}` to a caregiver. The sentence is assembled in component code from `heardStripCounty` / `heardStripChild` / the existing domain label, joined with `·`, and passed whole as `{parts}`.
3. **Only a *live* `heard` may replace the deterministic sentence.** `rankFamilyResourcesMock` sets `heard` to `rankHeardFallback` — the generic catalog caveat — so keying the swap on `rankedSet` alone made the zero-key demo *lose* the personalized sentence it had just computed. The swap is gated on `rankedSet.extraction === "live"`.
4. **Any school-stage hint counts, stated or inferred.** F2 distinguished a "stated hint" from an approximate one, but `extractFamilyBasics` marks *every* extracted birth year `approximate`, and the stage inference (age + a school mention) is the branch the existing seven-year-old journey depends on. Rule as built: **any** `schoolStage` hint wins; else age ≤ 4 defaults to `not_school_age`; else the stage is asked.
5. **`FamilyProfileForm` had ten hardcoded DOM ids.** Mounting it a second time inside the disclosure would have bound every label to whichever copy came first in the document — a real user-facing bug, invisible to any test that mounts one form. The ids are now `useId()`-scoped, **and** the disclosure's editor is mounted only while the disclosure is open, so the two copies never coexist.
6. **FR-5's "state is shared, not duplicated" holds only for persisted state.** `isSaved` / `isEnrolled` / `step` are props read from the store and do stay in sync across the two copies; the share-consent checkbox and the "shared" flag are per-instance `useState` in the card and do not. Tests assert cross-copy behavior through plan-step / save / enroll, never through share.
7. **`basicsPrefillApproxYear` is deleted, not kept.** Open question 5 predicted it would survive; its only consumer was inside the deleted confirm card and no natural reuse appeared, so keeping it would have violated FR-7's own no-orphans rule.
8. **`crisis:gate` cannot be byte-identical, and the script diffs nothing.** It stamps the report with the date and embeds vitest's `Start at` / `Duration` lines. FR-8 is discharged as: same 7 files, `Tests 334 passed (334)`, `## Result → PASS`, exit 0, and `git diff` showing only the two timing lines. Verified in that form.

Minor: the store is `src/state/store.tsx`, not `store.ts`; `profileProvenance` is required on the type and inserted immediately after `profile` in all four literals, because `loadStoredState` compares `JSON.stringify` per slice and a different key order would rewrite localStorage on every load.

### Post-implementation review, 2026-08-04

A five-lens adversarial review of the diff (safety, state/storage, spec fidelity, i18n/a11y, collateral damage) raised 19 candidates; each was independently re-verified by two skeptics prompted to refute it, and 16 survived. All were fixed. The ones worth recording, because each broke something this spec explicitly promised:

1. **The eligibility caveat vanished from every described-child path (high).** Two edits combined: the strip drops a non-live `heard`, and the section's `resourcesIntro` was gated on `latestInterview`. Both strings carry "check the program's own page — their rules are the ones that count", and on the deterministic path — the zero-key demo, every e2e run, and any ranking outage — neither rendered. Eleven eligibility-bearing cards with no eligibility caveat anywhere on the page. The intro is now gated on whether a *live* sentence is showing, which is exactly when it stood down before.
2. **Complete hints plus two interviews was an unrecoverable dead end (high).** Scoping auto-apply to `interviews.length === 1` meant a caregiver who tapped "Start over" and then wrote a complete description got no auto-apply *and* no answerable turn (all three fields were seeded, so `onComplete` never fired) — a permanently profile-less, persisted state. Auto-apply is now latched per interview id instead of per interview count.
3. **A mid-thread profile edit destroyed the conversation (medium, safety-adjacent).** The thread reset on any profile change. The strip's editor makes such edits reachable from inside the thread, so correcting a county could drop a pending follow-up and move an unacknowledged crisis banner out from the top of the page and under the composer. The thread now resets only on a language switch.
4. **The one-ask invariant was violated for a frame (FR-9).** The stale-step question was suppressed via an effect-driven signal, which blinked off between thread rounds and let a second question in. The signal is now monotonic per visit (`thread.status !== "idle"`), and FR-9 finally has the assertion test it always required.

Also fixed: the age ≤ 4 stage default existed only in the auto-apply memo, so the demo script's own two-year-old still got a school-stage turn; "See all {n}" counted the retrieval set rather than the eight cards the section renders; two mounted profile forms could each hold a draft and silently revert each other; `"{child}, about 1"` / `"de unos 1 años"` for the 0–3 population this app targets; `"Parece que:"` waiting for a verb that never arrives; two landmarks both named "Places that can help"; three resource cards floating under the composer with no strip on a reload; and one share producing two audit events when a resource was on screen twice.

Final state: `npx next lint` clean, **2817 unit tests passing** (baseline 2797), `next build` clean, `crisis:gate` PASS with 334 tests across 7 files unchanged, ladder e2e 53 passed / 1 skipped / 0 failed.
...
5. **Dead-string deletion** (`basicsPrefillTitle/Intro/Confirm/Change`, `resourcesFoundBelow*`, possibly `factsIntro`/`factsTitle` if fully superseded) — grep for consumers before removing; `factsTitle` may survive as the disclosure's internal heading. **Resolved at build time:** eight keys deleted, `factsTitle` kept as the disclosure heading (~17 assertions depend on it), `resourcesIntro` kept for the screen-only path, and `basicsPrefillApproxYear` deleted rather than kept — no reuse appeared and keeping it would have violated FR-7.

6. **The disclosure empties after a follow-up round (pre-existing, surfaced by this spec).** `reviewFacts` filters `family.facts` by the *latest* interview id, while `addFamilyInterview` dedupes re-extracted facts and keeps the first copy on its original interview. So a round-1 interview that surfaces no genuinely new fact leaves the strip's disclosure holding only the rationale, the editor, and the trust line. The old review block filtered identically, so this is not a regression — but this spec makes the disclosure the only in-thread home for fact cards, which makes it more visible. Everything remains in the journal below, with the same confirm action. Fixing it means scoping `reviewFacts` to the whole current conversation rather than the latest interview id; deliberately **not** done here because it changes what the review surface shows across rounds, which this spec's non-goals exclude.

7. **The e2e suite stubs `/api/family/interview` but not `/api/family/recommend`.** On a machine with a live key in `.env.local`, the ranking call goes out during the navigator journeys, so the strip renders a model-written sentence instead of the deterministic template and card order can shift. The journeys pass either way today, and the new FR-12 journey stubs recommend explicitly so its assertions are key-independent. Stubbing it suite-wide would make the whole gate deterministic and is worth doing — out of scope here because it changes journeys this spec does not otherwise touch.
